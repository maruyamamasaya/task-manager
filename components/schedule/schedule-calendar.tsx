...
const sunday = (date:string) => { const d=new Date(`${date}T00:00:00+09:00`); return addDays(date,-d.getUTCDay()); };
...
export function ScheduleCalendar({view,date,tasks,schedules,dayOffs}:{view:"month"|"week";date:string;tasks:Task[];schedules:TaskSchedule[];dayOffs:DayOff[]}) {
  ...
  const start=view==="week"?sunday(date):(()=>{const first=`${date.slice(0,7)}-01`;return addDays(first,-new Date(`${first}T00:00:00+09:00`).getUTCDay());})();
