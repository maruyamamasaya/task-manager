...
const formatDate = (date:string|null) => date ? `${Number(date.slice(5,7))}/${Number(date.slice(8,10))}` : "未設定";
const formatHours = (hours:number) => new Intl.NumberFormat("ja-JP", { maximumFractionDigits: 2 }).format(hours);
    ...
    {shown.length === 0 ? <EmptyState title="WBSはまだありません" description="作業を階層化して、プロジェクト全体の進捗を管理できます。" action={<div className="flex flex-col items-center gap-3"><Link href="/wbs/new" className="inline-flex min-h-9 items-center rounded-lg bg-indigo-600 px-3.5 text-sm font-medium text-white hover:bg-indigo-700">新規WBS</Link><button className="text-sm font-medium text-slate-600 hover:text-indigo-700" onClick={() => setJoinOpen(true)}>共有プロジェクトに参加</button></div>} /> :
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        {shown.map((project) => <article key={project.id} className="group relative rounded-xl border border-slate-200 bg-white transition-colors hover:border-indigo-300 hover:bg-slate-50/50">
          ...
          <dl className="mt-4 grid gap-2 rounded-lg border border-slate-200 bg-white p-3 text-sm">
            <ProjectSummaryRow label="期間" value={project.startDate || project.endDate ? `${formatDate(project.startDate)} - ${formatDate(project.endDate)}` : "未設定"} />
            <ProjectSummaryRow label="予定工数" value={`${formatHours(project.estimateHours)} h`} />
            <ProjectSummaryRow label="実績工数" value={`${formatHours(project.actualHours)} h`} />
          </dl>
...
function ProjectSummaryRow({label,value}:{label:string;value:string}) { return <div className="flex items-center justify-between gap-3"><dt className="text-xs font-medium text-slate-500">{label}</dt><dd className="font-semibold tabular-nums text-slate-800">{value}</dd></div>; }
