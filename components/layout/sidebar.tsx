...
export function Sidebar({ email }: { email: string }) {
  ...
  return <>
    ...
    <nav className="fixed inset-x-0 bottom-0 z-20 flex gap-1 overflow-x-auto border-t border-slate-200 bg-white/95 px-2 py-2 backdrop-blur md:hidden">{items.map(([label, href, icon]) => <Link key={href} href={href} aria-label={label} className={`min-w-16 flex-1 rounded-lg py-1 text-center text-[10px] ${pathname === href ? "bg-indigo-50 font-semibold text-indigo-700" : "text-slate-500"}`}><span className="block text-lg">{icon}</span>{label}</Link>)}</nav>
  </>;
}
