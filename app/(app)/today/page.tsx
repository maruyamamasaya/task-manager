...
function TaskSection({ title, tasks, kind }: { title: string; tasks: Task[]; kind: "overdue" | "today" | "done" }) {
  const tones = { overdue: "text-red-600", today: "text-indigo-600", done: "text-emerald-600" };
  return <section className="overflow-hidden rounded-xl border border-slate-200/80 bg-white"><header className="flex items-center justify-between border-b border-slate-100 px-4 py-3"><h2 className="text-sm font-semibold text-slate-900">{title}</h2><span className="rounded-md bg-slate-100 px-2 py-0.5 text-xs text-slate-500">{tasks.length}</span></header><div className="divide-y divide-slate-100">{tasks.length ? tasks.map(task => <Link href={`/tasks?task=${task.id}`} key={task.id} className="block px-4 py-3 transition-colors hover:bg-slate-50"><div className="flex items-start gap-2.5"><Icon name="empty" className={`mt-0.5 size-4 shrink-0 ${tones[kind]}`}/><div className="min-w-0 flex-1"><div className="flex items-center justify-between gap-3"><p className={`truncate text-sm font-medium ${kind === "done" ? "text-slate-500 line-through" : "text-slate-800"}`}>{task.title}</p><span className="text-xs font-medium text-slate-500">{kind === "done" ? "完了" : `${task.progress}%`}</span></div>{kind !== "done" && <Progress value={task.progress} className="mt-2"/>}<p className={`mt-1.5 text-xs ${kind === "overdue" ? "text-red-600" : "text-slate-400"}`}>{task.due_at ? formatTokyo(task.due_at, { month: "numeric", day: "numeric", hour: "2-digit", minute: "2-digit" }) : "期限なし"}</p></div></div></Link>) : <div className="px-4 py-8 text-center"><Icon name="empty" className="mx-auto size-6 text-slate-300"/><p className="mt-2 text-sm font-medium text-slate-600">タスクはありません</p><p className="mt-1 text-xs text-slate-400">該当するタスクがここに表示されます。</p></div>}</div></section>;
}
...
export default async function Page() {
  ...
  const tasks = all ?? [];
  const overdue = tasks.filter(task => task.status !== "done" && task.due_at! < bounds.start);
  const today = tasks.filter(task => task.status !== "done" && task.due_at! >= bounds.start && task.due_at! < bounds.end);
