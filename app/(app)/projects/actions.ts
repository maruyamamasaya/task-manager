...
export async function createProject(name:string,color:string){const {db,user}=await dbUser();const {error}=await db.from("projects").insert({name:name.trim(),color,user_id:user.id});if(error)return {error:error.message};revalidatePath("/projects");return {ok:true};}
