...
export async function updateMeeting(input:{id:string;name:string;startAt:string;duration:number}){const name=input.name.trim();if(!name)return {error:"会議名を入力してください。"};if(!Number.isInteger(input.duration)||input.duration<=0)return {error:"会議時間は1分以上の整数で入力してください。"};if(isPast(input.startAt))return {error:"過去の会議は編集できません。"};const {db}=await context();const endAt=new Date(new Date(input.startAt).getTime()+input.duration*60000).toISOString();const {error}=await db.from("meetings").update({name,start_at:input.startAt,end_at:endAt}).eq("id",input.id);if(error)return {error:error.message};refresh();return {ok:true};}
