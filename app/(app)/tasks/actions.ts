...
export async function importTasks(markdown: string, projectId?: string) {
  ...
  for (const [index, task] of parsed.entries()) { const { data, error } = await db.from("tasks").insert({ user_id: user.id, title: task.title, parent_id: task.parentIndex === null ? null : ids[task.parentIndex], project_id: projectId || null, status: task.done ? "done" : "todo", progress: task.done ? 100 : 0, completed_at: task.done ? new Date().toISOString() : null, sort_order: index }).select("id").single(); if (error) return { error: `${index + 1}行目: ${error.message}` }; ids.push(data.id); }
