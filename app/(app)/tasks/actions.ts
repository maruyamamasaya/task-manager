...
export async function createTasks(inputs: TaskInput[]) {
  const tasks = inputs
    .map((input) => ({ ...input, title: input.title.trim() }))
...
export async function saveProgress(id: string, progress: number, note: string) {
  ...
  const { error } = await db.rpc("update_task_progress", {
    target_id: id,
    next_progress: clampProgress(progress),
    progress_note: note,
  });
  if (error) return { error: error.message };
  refresh();
...
export async function importTasks(markdown: string, projectId?: string) {
  ...
  if (!parsed.length)
    return { error: "読み込めるチェックリストがありません。" };
  const { db, user } = await context();
  const ids: string[] = [];
  ...
  for (const [index, task] of parsed.entries()) {
    const { data, error } = await db
      ...
      .insert({
        ...
        completed_at:
          !folderIndexes.has(index) && task.done
      ...
      .select("id")
      .single();
    if (error) return { error: `${index + 1}行目: ${error.message}` };
    ids.push(data.id);
    if (!folderIndexes.has(index) && task.actualMinutes !== null) {
      ...
      if (logError)
        return { error: `${index + 1}行目の実績: ${logError.message}` };
    }
  }
  refresh();
