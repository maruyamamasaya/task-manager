...
import { leafProgress } from "@/lib/wbs/hierarchy";
...
export default async function WbsPage() {
  ...
  const summaries = ((projects ?? []) as WbsProject[]).map((project) => {
    ...
    return { ...project, role: project.owner_user_id === user!.id ? "owner" as const : memberRoles.get(project.id) ?? "viewer", itemCount: projectItems.length, progress: leafProgress(projectItems), overdueCount: projectItems.filter(item => item.end_date && item.end_date < today && item.status !== "completed" && item.status !== "on_hold").length, inProgressCount: projectItems.filter(item => item.status === "in_progress").length, onHoldCount: projectItems.filter(item => item.status === "on_hold").length };
  });
