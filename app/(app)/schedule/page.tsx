...
export default async function Page({searchParams}:{searchParams:Promise<{date?:string;view?:string}>}){
  ...
  return <><PageHeader title="Schedule" description="月・週・日の階層で、予定の密度から時間割まで確認できます。"/>
    ...
    {view==="day"?<DaySchedule date={date} tasks={(tasks??[]).filter(t=>t.status!=="done")} schedules={schedules??[]} logs={logs??[]} dayOff={dayOffs?.[0]}/>:<ScheduleCalendar view={view} date={date} tasks={tasks??[]} schedules={schedules??[]} dayOffs={dayOffs??[]}/>}</>;
}
