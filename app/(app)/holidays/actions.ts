...
export async function saveDayOff(formData:FormData){
  ...
  revalidatePath("/holidays");revalidatePath("/schedule");revalidatePath("/wbs", "layout");
}
export async function updateDayOff(formData:FormData){
  ...
  revalidatePath("/holidays");revalidatePath("/schedule");revalidatePath("/wbs", "layout");redirect("/holidays");
}
export async function deleteDayOff(formData:FormData){const db=await createClient();const id=String(formData.get("id")??"");const {error}=await db.from("day_offs").delete().eq("id",id);if(error)throw error;revalidatePath("/holidays");revalidatePath("/schedule");revalidatePath("/wbs", "layout");}
