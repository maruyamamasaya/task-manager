...
export default async function LoginPage({ searchParams }: { searchParams: Promise<{ next?: string }> }) {
  ...
  return <main className="grid min-h-screen place-items-center px-5 py-12">
    <section className="w-full max-w-md rounded-2xl border border-slate-200 bg-white p-8 shadow-sm sm:p-10">
      <div className="mb-8"><AppMark className="mb-5 size-11 drop-shadow-sm" /><h1 className="text-2xl font-bold">おかえりなさい</h1><p className="mt-2 text-sm text-slate-500">Task Manager にログインして、今日の仕事を始めましょう。</p></div>
      <LoginForm next={next} />
