...
test("reflection waiting detection",()=>{assert.equal(needsReflection({status:"done"}),true);assert.equal(needsReflection({status:"doing"}),false);assert.equal(needsReflection({status:"done"},{id:"r"} as never),false);});
